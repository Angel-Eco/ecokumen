...
function FormFacade(data)
{
    ...
    this.isEditMode = function()
    {
        return location.href.indexOf('https://formfacade.com/edit/')==0
            || location.href.indexOf('http://whatstarget.com/edit/')==0
            || location.href.indexOf('http://neartail.com/edit/')==0
            || location.href.indexOf('http://mailrecipe.com/edit/')==0
    ...
    this.jumptoSection = function(frm, secid, deftrg, trg)
    {
    ...
    var asDate = function(itm, date)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                var dt = new Date();
                if(b.length==3)
                    dt = new Date(b[0], b[1]-1, b[2]);
                else if(b.length==6)
                    dt = new Date(b[0], b[1]-1, b[2], b[3], b[4], b[5]);
                vl = toDate(dt);
            }
            vl.getMetadata = function(){ return itm; }
            return vl;
...
formFacade.langtext = {"Back":"AtrÃ¡s","Next":"Siguiente","Submit":"Enviar","Choose":"Elige","Email address":"DirecciÃ³n de correo electrÃ³nico","This is a required question":"Este campos es obligatorio","This question requires one response per row":"Esta pregunta requiere una respuesta por fila","This question requires at least one response per row":"Esta pregunta requiere de al menos una respuesta por fila","Please don't select more than one response per column":"Por favor, no seleccione mÃ¡s de una respuesta por columna","Invalid date":"Fecha no vÃ¡lida","Invalid time":"Hora no vÃ¡lida","Invalid duration":"DuraciÃ³n no vÃ¡lida","You partially filled this form $ago minutes ago":"CompletÃ³ este formulario parcialmente hace $ago minutos","Continue":"Seguir","Start over":"Comenzar de nuevo","Start":"Comenzar","Share":"Compartir","Share this form with others":"Comparta este formulario con otros","Done":"Listo","locale":"ES"}
